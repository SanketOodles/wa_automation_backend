...
const { uploadBase64ImageToZoho } = require('../utils/zoho.service');
const qrcode = require('qrcode-terminal');
const { createAttachments } = require('../utils/airtable.service');
const { where, Op } = require('sequelize');

/**
 * ===== MULTI-CLIENT WHATSAPP IMPLEMENTATION =====
 * This module provides functionality to manage multiple WhatsApp clients
 * for different users and accounts.
 */

// Store multiple clients and their QR codes
const clients = {};
const qrCodes = {};


/**
 * Creates a WhatsApp client for a specific user and account
 * @param {string} userId - Unique identifier for the user
 * @param {string} accountId - Identifier for the specific account
 * @returns {Object} The WhatsApp client instance
 */
function createClientForAccount(userId, accountId) {
  const sessionPath = path.join(__dirname, 'sessionStorage', `${userId}_${accountId}.json`);
  ...
  const client = new Client({
    authStrategy: new LocalAuth({
      dataPath: './sessionStorage',
      clientId: clientId,   // in futute changes in this line
    }),
    puppeteer: {
  ...
  client.on('loading_screen', (percent, message) => {
    console.log(`[${clientId}] LOADING SCREEN`, percent, message);
  });
    ...
    client.on('qr', async (qr) => {
        // NOTE: This event wil not be fired if a session is specified.
        console.log('QR RECEIVED', qr);
        qrcode.generate(qr, { small: true });
        // lawarish line globalQR = qr;
    });
...
client.on('message_create', async (msg) => {
    ...
    //console.log("message_create", JSON.stringify(msg));
    try {
        const payload = {
            message: msg.hasMedia ? msg._data.caption : msg.body,
            wa_message_id: msg.id.id,
            wa_from: msg.from,
            wa_to: msg.to,
            is_attachment: msg.hasMedia,
            is_forward: msg.isForwarded ? true : false,
            is_reaction: msg.hasReaction,
            is_quote_msg: msg.hasQuotedMsg
        };
        const chat = await db.Chat.create(payload);
        let ai_response_data;
        if (!msg.hasMedia) {
            ai_response_data = await ai.apis({ base64: '', data: msg.hasMedia ? msg._data.caption : msg.body });
            await db.Record.create({
                recordId: ai_response_data.id,
                from: msg.from
            });
        }
        ...
        if (msg.hasMedia) {
            ...
            if (media) {
                const extension = media.mimetype.split('/')[1]; // get file extension from mimetype
                ...
                const db_media = await db.Media.create(media_payload);

                // Save media as base64 decoded buffer
                // fs.writeFile(filePath, media.data, 'base64', function (err) {
                //     if (err) {
                ...
                //     }
                // });
                // const ai_response = await ai.apis({ base64: media.data, data: '' });
                // const zohoMedia = await uploadBase64ImageToZoho(ai_response_data.details.id, media.data, filename);
                // const zohoMedia = await uploadBase64ImageToZoho('4002118000052821001', media.data, filename);
                const twentySecondsAgo = new Date(Date.now() - 2 * 60 * 1000);
                const record = await db.Record.findOne({
                    where: {
                        createdAt: {
                            [Op.gte]: twentySecondsAgo
                        }
                    }
                });
                const airtableMedia = await createAttachments({ base64: media.data, filename: filename, recordId: record.dataValues.recordId, fieldName: 'Images' })
            }
            console.log("attachments", JSON.stringify(media));
...
/**
 * Gets or creates a client for a specific user and account
 * @param {string} userId - Unique identifier for the user
 * @param {string} accountId - Identifier for the specific account
 * @returns {Object} The WhatsApp client instance
 */
function getClient(userId, accountId) {
  const clientId = `${userId}_${accountId}`;
  ...
  return createClientForAccount(userId, accountId);
}

/**
 * Gets the QR code for a specific client
 * @param {string} userId - Unique identifier for the user
 * @param {string} accountId - Identifier for the specific account
 * @returns {string|null} The QR code or null if not available
 */
function getClientQR(userId, accountId) {
  const clientId = `${userId}_${accountId}`;
 ...
 * @returns {string[]} Array of active client IDs
 */
 ...
 * @param {string} accountId - Identifier for the specific account
 */
async function disconnectClient(userId, accountId) {
  const clientId = `${userId}_${accountId}`;
  if (clients[clientId]) {
    await clients[clientId].destroy();
    delete clients[clientId];
    delete qrCodes[clientId];
    console.log(`Client ${clientId} disconnected and removed`);
  }
}
